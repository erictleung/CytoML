...
test_that("diva--escape forward slash ",{
  ...
  stats <- gh_pop_stats_print(gs[[1]], type = "Freqofparent")[, value]
...
test_that("diva--tcell ",{
  ...
  parsedStats <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(parsedStats[, cytolib], parsedStats[,xml], tol = 5e-3)
  ...
  parsedStats <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(parsedStats[,cytolib], c(1,.73,.757,.305,.569), tol = 2e-2)
...
test_that("diva--2002-D-g001 ",{
  ...
  parsedStats[, cytolib := round(cytolib, digits = 3)]

  expect_equivalent(parsedStats[-1,list(node, cytolib)], expectRes[-1,list(node, openCyto.freq)], tol = 6e-3)
