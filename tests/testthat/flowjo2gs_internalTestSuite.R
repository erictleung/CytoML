...
test_that("verify the extend logic no longer needed for the gate defined in biexp scale from latest flowjo wsp output",{
  ...
  res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  
  expect_equal(res[, xml], res[, cytolib], tol = 0.0003)
...
test_that("flog-- offset and decades that expose the previous logGml2-based flog was wrong",{
  ...
  res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(res[, xml], res[, cytolib], tol = 0.01)
...
test_that("which.lines",{
  ...
  res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(res[, xml], res[, cytolib], tol = 0.04)
...
test_that("set T value properly through PnE instead of PnR for flog transform when FCS data is log scale",{
  ...
  res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(res[, xml], res[, cytolib], tol = 0.015)
...
test_that("handle the linear transform with maxRange = 0",{
  ...
  res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(res[, xml], res[, cytolib])
...
test_that("skip ManuallyIncludedSamples",{
  ...
  res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(res[, xml], res[, cytolib], tol = 0.009)
...
test_that("search reference node for boolean gate ",{
  ...
  expect_equal(res[-10, xml], res[-10, cytolib], tol = 0.006)
  expect_equal(res[10, cytolib], 164)
  ...
  res <- gh_pop_compare_stats(gh, type = "Freqofparent")
  expect_true(all(is.na(res[leaf.bool,  cytolib])))
  expect_equal(res[-leaf.bool, xml], res[-leaf.bool, cytolib], tol = 0.006)
  })
...
test_that("vertical ellipsoidGate for vX ",{
  ...
  expect_equal(res[, xml], res[, cytolib], tol = 0.02)
})
...
test_that("skip gains from FCS for vX ",{
 ...
 capture.output( gs <- flowjo_to_gatingset(ws, name=2))
  ...
  expect_equal(res[, xml], res[, cytolib], tol = 1e-3)
})
...
test_that("gate extension ",{
      ...
      res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")[xml >0, ]
      
      expect_equal(res[, xml], res[, cytolib], tol = 3e-2)
    })
...
test_that("curlyQuad gate1 ",{
      ...
      res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
      expect_equal(res[, xml], res[, cytolib], tol = 2e-3)
    })
...
test_that("curlyQuad gate1 ",{
      ...
      res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
      expect_equal(res[, xml], res[, cytolib], tol = 7e-3)
    })
...
test_that("curlyQuad gate ",{
      ...
      res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
      expect_equal(res[, xml], res[, cytolib], tol = 1.6e-2)
    })
...
test_that("EllipsoidGate defined on log-transformed channels ",{
      ...
      res <- gh_pop_compare_stats(gs[[1]])
      expect_equal(res[, xml], res[, cytolib], tol = 4e-3)
})
...
test_that("No gate extension ",{
      ...
      res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
      expect_gt(res[11, abs(xml - cytolib)], 0.1)
...
test_that("FCS searching path with symlink ",{
  ...
  res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
  expect_equal(res[, xml], res[, cytolib], tol = 9e-4)
})
...
test_that("Time gate2--when computed timestep is very different from $TIMESTEP ",{
      ...
      nodes <- gh_pop_compare_stats(gs[[1]], type = "Count")[xml > 0 , node]
      res <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")[node %in% nodes,  ]
      expect_equal(res[, xml], res[, cytolib], tol = 8e-3)
    })
test_that("Inverse function of flog ",{
      ...
      gh <- gs[[1]]
      thisCounts <- gs_pop_stats_print(gs, path = "auto")
      expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
      
      expect_equivalent(thisCounts[pop!= "root",], expectCounts[, list(name, Population, Count)])
...
test_that("v 10.0.6 - vX 1.8",{
      ...
      thisCounts <- gh_pop_compare_stats(gh, type = "Freqofparent")
...
test_that("v 10.0.7 - vX 20.0 (ellipsoidGate)",{
      ...
      thisCounts <- gh_pop_compare_stats(gh, type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 2e-2)
    })
...
#      gh <- gs[[1]]
#      expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
#      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")[, list(xml.count,openCyto.count, node)]
...
#      expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[,xml], thisCounts[,cytolib], tol = 7e-4)
    })
...
test_that("v 10.0.7 - vX 20.0 (PROVIDE/CyTOF) ellipseidGate (fasinh)",{
      ...
      expect_equal(thisCounts[, cytolib], thisCounts[, xml], tol = 0.04)
...
test_that("v 10.0.7r2 - vX 20.0 (NotNode)",{
      ...
      expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
      thisCounts <- gh_pop_compare_stats(gh)
      expect_equivalent(thisCounts[,c("xml", "node"), with = F], expectCounts[,c("flowJo.count", "node"), with = F])
    })
...
test_that("v 10.0.8r1 - vX 20.0 (OrNode)",{
  ...
  expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
  thisCounts <- gh_pop_compare_stats(gh)[, list(xml, node)]
  expect_equivalent(thisCounts, expectCounts[, list(flowJo.count, node)])
})
...
#      expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 0.025)
...
#      expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 0.03)
...
test_that("v 7.6.1- win 1.6 (use default biexp trans when channel-specific trans not found within its respective trans group )",{
      ...
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 0.015)
    })
...
test_that("v 7.6.5 - win 1.61 (PBMC)",{
      ...
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 9e-3)
...
test_that("v 7.6.5 - win 1.61 (sampNloc = 'sampleNode')",{
      ...
      gh <- gs[[1]]
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 2e-4)
...
test_that("v 9.0.1 - mac 2.0 (HVTN 080-0880)",{
      ...
      gh <- gs[[1]]
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 4e-3)
...
test_that("v 9.2 - mac 2.0 (ITN029)",{
      ...
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 7e-3)
...
test_that("v 9.4.2 - mac 2.0",{
      ...
      gh <- gs[[1]]
      thisCounts <- gh_pop_compare_stats(gh,  type = "Freqofparent")
      expect_equal(thisCounts[, xml], thisCounts[, cytolib], tol = 5e-3)
...
test_that("v 9.7.4 - mac 3.0",{
      ...
      gh <- gs[[1]]
      expectCounts <- fread(file.path(thisPath, "expectCounts.csv"))      
      thisCounts <- gh_pop_compare_stats(gh, path = "full")[, list(xml, node)]
      expectCounts[flowJo.count ==0, flowJo.count := -1]#fix the legacy counts
      expect_equal(thisCounts, expectCounts[, list(flowJo.count, node)], check.attributes = FALSE, tol = 3.3e-3)
      thisCounts <- gh_pop_compare_stats(gh, path = "full")[, list(cytolib, node)]
      expect_equal(thisCounts, expectCounts[, list(flowCore.count, node)], check.attributes = FALSE, tol = 3.3e-3)
    })
...
test_that("v 9.7.5 - mac 3.0 (no compensation and using calibrationIndex)",{
      ...
      thisCounts <- gh_pop_compare_stats(gh)
      expectCounts[flowJo.count ==0, flowJo.count := -1] #fix the legacy counts
      expect_equal(thisCounts[, list(cytolib, node)], expectCounts[, list(flowCore.count, node)], check.attributes = FALSE, tol = 2e-4)
    })
...
test_that("magnetic gates", {
  ...
  gs <- flowjo_to_gatingset(ws, name = 1)
  thisCounts <- gh_pop_compare_stats(gs[[1]], type = "Freqofparent")
